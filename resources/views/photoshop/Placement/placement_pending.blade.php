...
@endif
  			...
  			<div class="col-md-12 widget-holder content-area">
  				<div class="widget-bg">
  					...
  					<div class="widget-body clearfix dataTable-length-top-0">
	                    ...
	                    <table class="table table-striped table-center word-break mt-0"   id="placementpendinglist" >
  							<thead>
  								<tr class="bg-primary">
								  <th>Sr No</th>
  									<th>Sku</th>
	...
	{{$item->name}}
		...
		<td>	<select name="status" onchange="placementdone(this.value)" class="form-control" style="height:20px;width:150px;float: left;">
...
<!-- /.main-wrappper -->
<input type="hidden" id="placementpendingAjax" value="<?=URL::to('Photoshop/placement/pending_ajax_list');?>">
...
$.ajaxSetup({
	...
	var table = $('#placementpendinglist').DataTable({
  ...
  "order": [[ 0, "desc" ]],
  "deferLoading": <?=count($list)?>,
  "processing": true,
  "serverSide": true,
  "searching": false,
  "serverMethod": "post",
  "ajax":{
    "url": $("#placementpendingAjax").val(),
...
if(status !="2"){
  swal({
              ...
              if(data.value){
               ...
               $.ajax({
          url: "<?=URL::to('Photoshop/Placement/pending');?>",
          type:"POST",
          data:{
            "_token": "{{ csrf_token() }}",
            status:status,
